...
function familyGroup(label,people){return people.length?`<section class="person-family-group"><h3>${esc(label)}</h3><div class="v17-family-group-grid">${people.map(person=>personButton(person)).join('')}</div></section>`:'';}
...
export function renderNativePerson(id){
  ...
  return`<div class="v17-native person-profile" data-v17-native="person" data-person-id="${esc(person.id)}"><a class="v17-back" href="#people">← People</a><header class="person-header"><span class="v17-avatar hero" data-v17-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><div><p class="eyebrow">${esc(branch)} FAMILY</p><h2>${esc(cleanName(person.name))}</h2><p class="person-dates">${esc(publicDates(person))}</p><p class="person-context">${esc([person.role,...places.slice(0,1)].filter(Boolean).join(' · ')||'Family member')}</p>${qualified?`<div class="v17-qualified-state">Research status ${stateBadges(person.state)} ${esc(person.state)}</div>`:''}<div class="person-actions"><a class="action primary" href="${esc(treeHref(person.id))}">View in family tree</a><a class="action" href="#media">Browse family photos</a></div></div></header><nav class="person-nav" aria-label="Person page sections"><a href="#v17-family">Family</a><a href="#v17-life">Life</a><a href="#v17-photos">Photos</a><a href="#v17-research">Research</a></nav><section id="v17-family" class="person-section"><div class="v17-section-head"><div><p class="eyebrow">FAMILY</p><h2>Immediate family</h2></div><a href="${esc(treeHref(person.id))}">View in tree ↗</a></div><div class="v17-family-groups">${familyGroup('Parents',family.parents)}${familyGroup('Spouse / partner',family.spouses)}${familyGroup('Children',family.children)}${familyGroup('Siblings',family.siblings.slice(0,6))}</div></section><section id="v17-life" class="person-section"><div class="v17-section-head"><div><p class="eyebrow">LIFE</p><h2>Life and places</h2><p>${person.living?'Public view protects birth and location details for living family members.':'Chronology shown from source-controlled family records; qualified events remain visibly qualified.'}</p></div></div>${!person.living&&places.length?`<div class="person-places">${places.map(place=>`<a href="#migration">${esc(place)}</a>`).join('')}</div>`:''}${events.length?`<div class="v17-life-timeline">${events.map(timelineCard).join('')}</div>`:'<p class="muted">No public-safe timeline events are currently linked to this person.</p>'}</section><section id="v17-photos" class="person-section"><div class="v17-section-head"><div><p class="eyebrow">PHOTOS & DOCUMENTS</p><h2>Family archive</h2></div><a href="#media">Browse all media ↗</a></div><div class="person-gallery" data-v17-person-gallery="${esc(person.id)}"><p class="muted">Loading public-safe family media…</p></div></section><section id="v17-research" class="v17-person-research"><div><p class="eyebrow">RESEARCH CENTER</p><h2>The records behind this person</h2><p>${counts.claims} linked claim${counts.claims===1?'':'s'} · ${counts.sources} source${counts.sources===1?'':'s'} · ${counts.relationships} family relationship${counts.relationships===1?'':'s'}. Evidence states remain source-controlled.</p></div><a class="action" href="#research">Open Research Center ↗</a></section></div>`;
}
