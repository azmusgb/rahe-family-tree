...
function personButton(person,label=''){
  if(!person)return'';
  return`<button type="button" class="relative-card" data-person="${esc(person.id)}"><span class="v17-avatar small" data-v17-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><span>${label?`<small>${esc(label)}</small>`:''}<b>${esc(cleanName(person.name))}</b><em>${esc(publicDates(person))}</em></span></button>`;
}
...
function homeTreePreview(){
  ...
  return`<section class="home-tree" aria-labelledby="v17-tree-preview-title"><div class="family-section-head"><div><p class="eyebrow">FAMILY CONNECTIONS</p><h2 id="v17-tree-preview-title">See how the family connects</h2></div><a href="#tree">Explore the full tree ↗</a></div><div class="home-tree-canvas">${parents.length?`<div class="home-tree-row parents">${parents.map(parent=>personButton(parent,'Parent')).join('')}</div><i aria-hidden="true"></i>`:''}<div class="home-tree-row focus">${personButton(focus,'Focus')}${spouse?personButton(spouse,'Spouse / partner'):''}</div>${children.length?`<i aria-hidden="true"></i><div class="home-tree-row children">${children.map(child=>personButton(child,'Child')).join('')}</div>`:''}</div></section>`;
}
function homeMomentCard(event,index){
  ...
  return`<article class="home-story-moment ${index===0?'featured':''}"><time>${esc(eventYear(event))}</time><div><span>${esc(eventType(event))}</span><h3>${esc(eventText(event))}</h3>${people?`<p>${esc(people)}</p>`:''}${place?`<small>${esc(place)}</small>`:''}</div></article>`;
}
function homePersonCard(person){return`<article class="home-person"><button type="button" data-person="${esc(person.id)}"><span class="v17-avatar" data-v17-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><span><small>${esc(personBranch(person))}</small><b>${esc(cleanName(person.name))}</b><em>${esc(publicDates(person))}</em><p>${esc(relationshipLine(person))}</p></span><i aria-hidden="true">→</i></button></article>`;}
...
export function renderNativeHome(){
  ...
  return`<div class="v17-native family-home" data-v17-native="home"><section class="family-home-hero"><div><p class="eyebrow">THE RAHE FAMILY</p><h2>The Rahe family, connected.</h2><p>Explore the people, relationships, photographs, places, and stories preserved in the family archive.</p><div class="family-primary-actions"><a class="action primary" href="#tree">Explore family tree</a><a class="action" href="#people">Browse people</a><a class="action" href="#media">Photos & documents</a></div><div class="home-metrics"><span><b>${people.length}</b><small>people</small></span><span><b>${branches.length}</b><small>branches</small></span><span><b>${rels.length}</b><small>family connections</small></span></div></div></section>${homeTreePreview()}${moments.length?`<section class="home-story"><div class="family-section-head"><div><p class="eyebrow">FROM THE FAMILY STORY</p><h2>Across generations and places</h2><p>Supported moments drawn from the source-controlled family record.</p></div><a href="#stories">Explore all stories ↗</a></div><div class="home-story-moments">${moments.map(homeMomentCard).join('')}</div>${places.length?`<div class="home-place-strip"><span>Places in the family story</span>${places.map(place=>`<a href="#migration">${esc(place)}</a>`).join('')}</div>`:''}</section>`:''}<section class="home-featured-people"><div class="family-section-head"><div><p class="eyebrow">PEOPLE</p><h2>Meet the family</h2></div><a href="#people">Browse everyone ↗</a></div><div class="home-people-grid">${featured.map(homePersonCard).join('')}</div></section><section class="home-media" data-v17-home-gallery hidden></section><section class="home-research-door"><div><p class="eyebrow">RESEARCH CENTER</p><h2>How do we know?</h2><p>Sources, evidence states, unresolved identities, conflicts, and research targets live in the dedicated research workspace.</p></div><a class="action" href="#research">Open Research Center ↗</a></section></div>`;
}
