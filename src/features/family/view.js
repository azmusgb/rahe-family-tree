...
function personButton(person,label=''){
  if(!person)return'';
  return`<button type="button" class="ui-relative" data-person="${esc(person.id)}"><span class="ui-avatar small" data-ui-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><span>${label?`<small>${esc(label)}</small>`:''}<b>${esc(cleanName(person.name))}</b><em>${esc(publicDates(person))}</em></span></button>`;
}
function familyGroup(label,people){return people.length?`<section class="ui-family-group"><h3>${esc(label)}</h3><div class="ui-family-group-grid">${people.map(person=>personButton(person)).join('')}</div></section>`:'';}
...
function homeTreePreview(){
  ...
  return`<section class="ui-home-tree" aria-labelledby="ui-tree-view-preview-title"><div class="ui-section-head"><div><p class="eyebrow">FAMILY CONNECTIONS</p><h2 id="ui-tree-view-preview-title">See how the family connects</h2></div><a href="#tree">Explore the full tree ↗</a></div><div class="ui-family-home-tree-canvas">${parents.length?`<div class="ui-home-tree-row parents">${parents.map(parent=>personButton(parent,'Parent')).join('')}</div><i aria-hidden="true"></i>`:''}<div class="ui-home-tree-row focus">${personButton(focus,'Focus')}${spouse?personButton(spouse,'Spouse / partner'):''}</div>${children.length?`<i aria-hidden="true"></i><div class="ui-home-tree-row children">${children.map(child=>personButton(child,'Child')).join('')}</div>`:''}</div></section>`;
}
function homeMomentCard(event,index){
  ...
  return`<article class="ui-family-story-moment ${index===0?'featured':''}"><time>${esc(eventYear(event))}</time><div><span>${esc(eventType(event))}</span><h3>${esc(eventText(event))}</h3>${people?`<p>${esc(people)}</p>`:''}${place?`<small>${esc(place)}</small>`:''}</div></article>`;
}
function homePersonCard(person){return`<article class="ui-family-home-person"><button type="button" data-person="${esc(person.id)}"><span class="ui-avatar" data-ui-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><span><small>${esc(personBranch(person))}</small><b>${esc(cleanName(person.name))}</b><em>${esc(publicDates(person))}</em><p>${esc(relationshipLine(person))}</p></span><i aria-hidden="true">→</i></button></article>`;}
...
export function renderNativeHome(){
  ...
  return`<div class="ui-native ui-home-view" data-ui-native="home"><section class="ui-family-home-hero"><div><p class="eyebrow">THE RAHE FAMILY</p><h2>The Rahe family, connected.</h2><p>Explore the people, relationships, photographs, places, and stories preserved in the family archive.</p><div class="ui-primary-actions"><a class="action primary" href="#tree">Explore family tree</a><a class="action" href="#people">Browse people</a><a class="action" href="#media">Photos & documents</a></div><div class="ui-home-metrics"><span><b>${people.length}</b><small>people</small></span><span><b>${branches.length}</b><small>branches</small></span><span><b>${rels.length}</b><small>family connections</small></span></div></div></section>${homeTreePreview()}${moments.length?`<section class="ui-home-story"><div class="ui-section-head"><div><p class="eyebrow">FROM THE FAMILY STORY</p><h2>Across generations and places</h2><p>Supported moments drawn from the source-controlled family record.</p></div><a href="#stories">Explore all stories ↗</a></div><div class="ui-family-story-moments">${moments.map(homeMomentCard).join('')}</div>${places.length?`<div class="ui-place-strip"><span>Places in the family story</span>${places.map(place=>`<a href="#migration">${esc(place)}</a>`).join('')}</div>`:''}</section>`:''}<section class="ui-featured-people"><div class="ui-section-head"><div><p class="eyebrow">PEOPLE</p><h2>Meet the family</h2></div><a href="#people">Browse everyone ↗</a></div><div class="ui-home-people-grid">${featured.map(homePersonCard).join('')}</div></section><section class="ui-home-media" data-ui-home-view-gallery hidden></section><section class="ui-research-door"><div><p class="eyebrow">RESEARCH CENTER</p><h2>How do we know?</h2><p>Sources, evidence states, unresolved identities, conflicts, and research targets live in the dedicated research workspace.</p></div><a class="action" href="#research">Open Research Center ↗</a></section></div>`;
}

function branchChip(branch,selected){return`<button type="button" class="ui-branch-chip ${selected===branch?'active':''}" data-branch="${esc(branch)}" aria-pressed="${selected===branch?'true':'false'}">${esc(branch)}</button>`;}
function peopleCard(person){return`<article class="ui-family-person-card"><button type="button" data-person="${esc(person.id)}"><span class="ui-avatar" data-ui-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><span class="ui-person-card-copy"><small>${esc(personBranch(person))}</small><b>${esc(cleanName(person.name))}</b><em>${esc(publicDates(person))}</em><p>${esc(relationshipLine(person))}</p></span><i aria-hidden="true">→</i></button></article>`;}
export function renderNativePeople(){
  ...
  return`<div class="ui-native ui-people-view" data-ui-native="people"><header class="ui-page-intro"><div><p class="eyebrow">FAMILY DIRECTORY</p><h2>Meet the people in the family</h2><p>Browse relatives by name or branch, then open a person to follow their family, places, life events, and photographs.</p></div><span><b>${people.length}</b><small>${selected?'in this branch':'people shown'}</small></span></header><nav class="ui-family-branch-browser" aria-label="Browse people by family branch"><button type="button" class="ui-branch-chip ${selected?'':'active'}" data-branch="" aria-pressed="${selected?'false':'true'}">All</button>${branches.map(branch=>branchChip(branch,selected)).join('')}</nav>${summary?`<section class="ui-branch-summary"><div><p class="eyebrow">${esc(selected.toUpperCase())} FAMILY</p><h3>${summary.people.length} ${summary.people.length===1?'person':'people'} in this branch</h3><p>${summary.years.length?`${Math.min(...summary.years)}–${Math.max(...summary.years)}`:'Dates vary by person'}${summary.places.length?` · ${esc(summary.places.join(' · '))}`:''}</p></div><div><a href="#stories">Stories</a><a href="#migration">Places</a></div></section>`:''}<div class="ui-people-grid">${people.map(peopleCard).join('')}</div>${people.length?`<p class="ui-directory-foot">Showing ${people.length} of ${all.length} people in the public-safe family directory.</p>`:'<div class="empty">No people match the current filters.</div>'}</div>`;
}

function timelineCard(event){
  const state=eventState(event),place=eventPlaces(event)[0]||'',qualified=state!=='SUPPORTED';
  return`<article class="ui-life-event state-${stateClass(state)}"><time>${esc(eventDate(event))}</time><div><span>${esc(eventType(event))}${qualified?` · ${stateBadges(state)}`:''}</span><h3>${esc(eventText(event))}</h3>${place?`<p>${esc(place)}</p>`:''}</div></article>`;
}
...
export function renderNativePerson(id){
  ...
  return`<div class="ui-native ui-person" data-ui-native="person" data-person-id="${esc(person.id)}"><a class="ui-back" href="#people">← People</a><header class="person-header"><span class="ui-avatar hero" data-ui-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><div><p class="eyebrow">${esc(branch)} FAMILY</p><h2>${esc(cleanName(person.name))}</h2><p class="ui-person-dates">${esc(publicDates(person))}</p><p class="ui-person-context">${esc([person.role,...places.slice(0,1)].filter(Boolean).join(' · ')||'Family member')}</p>${qualified?`<div class="ui-qualified-state">Research status ${stateBadges(person.state)} ${esc(person.state)}</div>`:''}<div class="ui-person-actions"><a class="action primary" href="${esc(treeHref(person.id))}">View in family tree</a><a class="action" href="#media">Browse family photos</a></div></div></header><nav class="ui-person-nav" aria-label="Person page sections"><a href="#v17-family">Family</a><a href="#v17-life">Life</a><a href="#v17-photos">Photos</a><a href="#v17-research">Research</a></nav><section id="v17-family" class="ui-person-section"><div class="ui-section-head"><div><p class="eyebrow">FAMILY</p><h2>Immediate family</h2></div><a href="${esc(treeHref(person.id))}">View in tree ↗</a></div><div class="ui-family-groups">${familyGroup('Parents',family.parents)}${familyGroup('Spouse / partner',family.spouses)}${familyGroup('Children',family.children)}${familyGroup('Siblings',family.siblings.slice(0,6))}</div></section><section id="v17-life" class="ui-person-section"><div class="ui-section-head"><div><p class="eyebrow">LIFE</p><h2>Life and places</h2><p>${person.living?'Public view protects birth and location details for living family members.':'Chronology shown from source-controlled family records; qualified events remain visibly qualified.'}</p></div></div>${!person.living&&places.length?`<div class="ui-person-places">${places.map(place=>`<a href="#migration">${esc(place)}</a>`).join('')}</div>`:''}${events.length?`<div class="ui-life-timeline">${events.map(timelineCard).join('')}</div>`:'<p class="muted">No public-safe timeline events are currently linked to this person.</p>'}</section><section id="v17-photos" class="ui-person-section"><div class="ui-section-head"><div><p class="eyebrow">PHOTOS & DOCUMENTS</p><h2>Family archive</h2></div><a href="#media">Browse all media ↗</a></div><div class="ui-person-gallery" data-ui-person-gallery="${esc(person.id)}"><p class="muted">Loading public-safe family media…</p></div></section><section id="v17-research" class="ui-person-research"><div><p class="eyebrow">RESEARCH CENTER</p><h2>The records behind this person</h2><p>${counts.claims} linked claim${counts.claims===1?'':'s'} · ${counts.sources} source${counts.sources===1?'':'s'} · ${counts.relationships} family relationship${counts.relationships===1?'':'s'}. Evidence states remain source-controlled.</p></div><a class="action" href="#research">Open Research Center ↗</a></section></div>`;
}

export function wrapNativeTree(html){return`<div class="ui-native ui-tree-view" data-ui-native="tree">${html}</div>`;}
