...
import{hydrateNativeFamily}from'./view.js';
import{branchHref}from'./branches.js';
...
function personCard(person,branch){return`<article class="ui-family-home-person ui-home-featured-person"><button type="button" data-person="${esc(person.id)}"><span class="ui-avatar" data-ui-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><span><small>${esc(branch||personBranches(person)[0]||'Family')}</small><b>${esc(String(person.name||'').replace(/\s*\/.*$/,'').trim())}</b><em>${person.living?'Living — details protected':esc(person.dates||'Dates not recorded')}</em><p>Open this person and follow their connected family.</p></span><i aria-hidden="true">→</i></button></article>`;}

function installHomeBranchIndex(root){
  if(root.querySelector('.ui-home-branch-preview'))return;
  const hero=root.querySelector('.ui-family-home-hero'),branches=branchNames(),rels=activeRelationships();if(!hero||!branches.length)return;
  const cards=branches.map(branch=>{const members=displayPeople().filter(person=>personBranches(person).includes(branch)),rep=branchRepresentative(branch,rels);return`<a class="ui-branch-card" href="${esc(branchHref(branch))}"><span>${esc(branch)}</span><b>${members.length} ${members.length===1?'person':'people'}</b>${rep?`<small>Start with ${esc(String(rep.name||'').replace(/\s*\/.*$/,'').trim())}</small>`:''}</a>`;}).join('');
  const section=document.createElement('section');section.className='ui-home-branch-preview';section.innerHTML=`<div class="ui-section-head"><div><p class="eyebrow">FAMILY BRANCHES</p><h2>One archive, many connected lines</h2><p>Every documented branch is a peer part of the family history. Open a branch for its people, places, stories, and records.</p></div><a href="#people">Browse all people ↗</a></div><div class="ui-branch-grid">${cards}</div>`;hero.insertAdjacentElement('afterend',section);
}
function installBalancedHomeTree(root){
  const section=root.querySelector('.ui-home-tree');if(!section||section.dataset.v172Unified==='true')return;
  ...
  const button=(person,label)=>`<button type="button" class="ui-relative" data-person="${esc(person.id)}"><span class="ui-avatar small" data-ui-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><span><small>${esc(label)}</small><b>${esc(String(person.name||'').replace(/\s*\/.*$/,'').trim())}</b><em>${person.living?'Living — details protected':esc(person.dates||'Dates not recorded')}</em></span></button>`;
  section.dataset.v172Unified='true';
  section.innerHTML=`<div class="ui-section-head"><div><p class="eyebrow">FAMILY CONNECTIONS</p><h2>Navigate the connected family network</h2><p>Start from a well-connected person, then move freely across branches, generations, spouses, parents, and descendants.</p></div><a href="${esc(treeHref(focus.id,'connected'))}">Explore the connected tree ↗</a></div><div class="ui-family-home-tree-canvas ui-home-tree-preview-canvas">${parents.length?`<div class="ui-home-tree-row parents">${parents.map(person=>button(person,'Parent')).join('')}</div><i aria-hidden="true"></i>`:''}<div class="ui-home-tree-row focus">${button(focus,'Network starting point')}${spouse?button(spouse,'Spouse / partner'):''}</div>${children.length?`<i aria-hidden="true"></i><div class="ui-home-tree-row children">${children.map(person=>button(person,'Child')).join('')}</div>`:''}</div>`;
}
function installBalancedFeatured(root){
  const section=root.querySelector('.ui-featured-people'),grid=section?.querySelector('.ui-home-people-grid');if(!section||!grid||grid.dataset.v172Balanced==='true')return;
  const featured=balancedFeatured(6);if(!featured.length)return;
  const heading=section.querySelector('.ui-section-head h2');if(heading)heading.textContent='Meet people across the branches';
  const eyebrow=section.querySelector('.ui-section-head .eyebrow');if(eyebrow)eyebrow.textContent='ACROSS THE FAMILY';
...
function installTreeBranchNavigator(root){
  if(root.querySelector('.ui-tree-branches'))return;
  ...
  const nav=document.createElement('section');nav.className='ui-tree-branches';nav.innerHTML=`<div><span class="eyebrow">EXPLORE THE FAMILY</span><b>Jump to a branch or return to the connected network</b></div><div class="ui-tree-branch-actions">${neutral?`<button type="button" class="ui-whole-family" data-v172-tree-connected data-focus="${esc(neutral.id)}">Connected family</button>`:''}${buttons}</div>`;anchor.insertAdjacentElement('beforebegin',nav);
}
...
async function hydrateTreePortraits(root){
  ...
  root.querySelectorAll('.graph-node[data-person]').forEach(node=>{
    const id=node.dataset.person,person=personById(id);if(!id||person?.living||node.querySelector('.ui-node-photo'))return;
    ...
    const image=document.createElementNS('http://www.w3.org/2000/svg','image');image.classList.add('ui-node-photo');image.setAttribute('x','15');image.setAttribute('y','18');image.setAttribute('width','42');image.setAttribute('height','42');image.setAttribute('preserveAspectRatio','xMidYMid slice');image.setAttribute('clip-path',`url(#${clipId})`);image.setAttribute('href',`/api/media?file=${encodeURIComponent(item.id)}`);image.setAttribute('aria-hidden','true');
    const initialsNode=node.querySelector('.node-initials');(initialsNode||node.firstChild)?.before?.(image);if(initialsNode)initialsNode.classList.add('ui-has-photo');
  });
}
