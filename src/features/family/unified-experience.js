...
function personCard(person,branch){return`<article class="home-person v172-home-person"><button type="button" data-person="${esc(person.id)}"><span class="v17-avatar" data-v17-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><span><small>${esc(branch||personBranches(person)[0]||'Family')}</small><b>${esc(String(person.name||'').replace(/\s*\/.*$/,'').trim())}</b><em>${person.living?'Living — details protected':esc(person.dates||'Dates not recorded')}</em><p>Open this person and follow their connected family.</p></span><i aria-hidden="true">→</i></button></article>`;}
...
function installBalancedHomeTree(root){
  const section=root.querySelector('.home-tree');if(!section||section.dataset.v172Unified==='true')return;
  ...
  const button=(person,label)=>`<button type="button" class="relative-card" data-person="${esc(person.id)}"><span class="v17-avatar small" data-v17-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><span><small>${esc(label)}</small><b>${esc(String(person.name||'').replace(/\s*\/.*$/,'').trim())}</b><em>${person.living?'Living — details protected':esc(person.dates||'Dates not recorded')}</em></span></button>`;
  section.dataset.v172Unified='true';
  section.innerHTML=`<div class="family-section-head"><div><p class="eyebrow">FAMILY CONNECTIONS</p><h2>Navigate the connected family network</h2><p>Start from a well-connected person, then move freely across branches, generations, spouses, parents, and descendants.</p></div><a href="${esc(treeHref(focus.id,'connected'))}">Explore the connected tree ↗</a></div><div class="home-tree-canvas v172-home-tree-canvas">${parents.length?`<div class="home-tree-row parents">${parents.map(person=>button(person,'Parent')).join('')}</div><i aria-hidden="true"></i>`:''}<div class="home-tree-row focus">${button(focus,'Network starting point')}${spouse?button(spouse,'Spouse / partner'):''}</div>${children.length?`<i aria-hidden="true"></i><div class="home-tree-row children">${children.map(person=>button(person,'Child')).join('')}</div>`:''}</div>`;
}
function installBalancedFeatured(root){
  const section=root.querySelector('.home-featured-people'),grid=section?.querySelector('.home-people-grid');if(!section||!grid||grid.dataset.v172Balanced==='true')return;
