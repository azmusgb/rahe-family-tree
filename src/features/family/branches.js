...
export function renderFamiliesIndex(){
  ...
  const cards=branches.map(branch=>{const summary=branchSummary(branch),range=summary.years.length?`${Math.min(...summary.years)}–${Math.max(...summary.years)}`:'Dates vary by person';return`<a class="v175-family-card" href="${branchHref(branch)}"><span class="eyebrow">FAMILY BRANCH</span><h2>${esc(branch)}</h2><p>${summary.people.length} ${summary.people.length===1?'person':'people'} · ${esc(range)}</p>${summary.places[0]?`<small>${esc(summary.places[0].place)}</small>`:''}<b>Explore branch →</b></a>`;}).join('');
  return`<div class="v17-native v175-families" data-v17-native="families"><header class="v175-families-hero"><div><p class="eyebrow">CONNECTED FAMILY LINES</p><h1>Explore the family by branch.</h1><p>Each branch brings together its people, family connections, supported chronology, places, and unresolved questions without flattening the evidence behind them.</p></div><a class="action primary" href="#tree">View connected tree</a></header><section class="v175-family-grid" aria-label="Family branches">${cards}</section><section class="v17-research-door"><div><p class="eyebrow">EVIDENCE & RESEARCH</p><h2>Branch stories stay tied to the record.</h2><p>Provisional and unresolved relationships remain visibly qualified. Rejected relationships never become active family connections.</p></div><a class="action" href="#research">Open Research Center ↗</a></section></div>`;
}

export function renderFamilyBranch(branch=routeBranchName()){
  const valid=branchNames().includes(branch);if(!valid)return`<div class="v17-native v175-branch" data-v17-native="${branchMarker(branch)}"><section class="empty"><h1>Family branch not found</h1><p>This branch is not present in the public-safe family directory.</p><a class="action" href="#families">Browse family branches</a></section></div>`;
  ...
  return`<div class="v17-native v175-branch" data-v17-native="${branchMarker(branch)}"><nav class="v175-branch-back" aria-label="Family branch breadcrumb"><a href="#families">All families</a><span aria-hidden="true">/</span><b>${esc(branch)}</b></nav><header class="v175-branch-hero"><div><p class="eyebrow">${esc(branch.toUpperCase())} FAMILY</p><h1>${esc(branch)} family</h1><p>${summary.people.length} ${summary.people.length===1?'person':'people'} in the public-safe archive · ${esc(range)}.</p><div class="v175-branch-actions">${anchor?`<a class="action primary" href="${esc(treeHref(anchor,'family'))}">View ${esc(branch)} family in tree</a>`:''}<a class="action" href="#media">Browse photos & documents</a></div></div><div class="v175-branch-metrics"><span><b>${summary.people.length}</b><small>people</small></span><span><b>${summary.events.length}</b><small>linked records</small></span><span><b>${summary.connected.length}</b><small>connected branches</small></span></div></header>${featured.length?`<section class="v175-branch-section"><div class="v17-section-head"><div><p class="eyebrow">PEOPLE</p><h2>People in the ${esc(branch)} family</h2><p>Open a person to follow their immediate family, chronology, places, photographs, and source-backed record.</p></div><a href="#" data-v175-people-link="${esc(branch)}">Browse directory ↗</a></div><div class="v175-branch-people">${featured.map(personCard).join('')}</div></section>`:''}${timeline.length?`<section class="v175-branch-section"><div class="v17-section-head"><div><p class="eyebrow">FAMILY STORY</p><h2>Supported moments in this branch</h2><p>Only public-safe, non-rejected records are surfaced here; qualified evidence remains qualified elsewhere in the archive.</p></div><a href="#timeline">Full timeline ↗</a></div><div class="v175-branch-timeline">${timeline.map(eventCard).join('')}</div></section>`:''}${summary.places.length?`<section class="v175-branch-section"><div class="v17-section-head"><div><p class="eyebrow">PLACES</p><h2>Places in the ${esc(branch)} family story</h2></div><a href="#migration">Explore places ↗</a></div><div class="v175-place-grid">${summary.places.map(({place,count})=>`<a href="#migration"><b>${esc(place)}</b><small>${count} linked public-safe record${count===1?'':'s'}</small></a>`).join('')}</div></section>`:''}${summary.connected.length?`<section class="v175-branch-section"><div class="v17-section-head"><div><p class="eyebrow">CONNECTED FAMILIES</p><h2>Where this branch connects</h2></div><a href="#families">All families ↗</a></div><div class="v175-connected-grid">${summary.connected.slice(0,8).map(({name,count})=>`<a href="${branchHref(name)}"><b>${esc(name)}</b><small>${count} active family connection${count===1?'':'s'}</small></a>`).join('')}</div></section>`:''}${summary.questions.length?`<section class="v175-branch-section v175-open-questions"><div class="v17-section-head"><div><p class="eyebrow">OPEN QUESTIONS</p><h2>Still being resolved</h2><p>These relationships remain provisional or unresolved. This page does not promote them.</p></div><a href="#research">Research Center ↗</a></div><div class="v175-question-grid">${summary.questions.map(questionCard).join('')}</div></section>`:''}<section class="v17-research-door"><div><p class="eyebrow">RESEARCH CENTER</p><h2>See the evidence behind the ${esc(branch)} family.</h2><p>Sources, evidence states, conflicts, negative searches, and acquisition targets remain available in the research workspace.</p></div><a class="action" href="#research">Open Research Center ↗</a></section></div>`;
}
