...
function card(event){
  ...
  return`<article class="ui-story-card ${stateClass(state)}"><div class="ui-story-year">${esc(year||'—')}</div><div><div class="v16-story-kicker"><p class="eyebrow">${esc(eventTypeOf(event).toUpperCase())}</p><span class="v16-story-state ${stateClass(state)}">${esc(state)}</span></div><h3>${esc(titleOf(event))}</h3><p class="v16-story-date">${esc(displayDate(event))}</p>${names?`<p class="v16-story-people">${esc(names)}</p>`:''}${places.length?`<p class="v16-story-place">${esc(places.slice(0,2).join(' → '))}</p>`:''}${sourceIds.length?`<small>Source reference ${esc(sourceIds.join(' · '))}</small>`:'<small>Source-controlled event from the canonical research model</small>'}</div></article>`;
}
...
export function renderStories(){
  ...
  return`<section class="ui-stories" aria-labelledby="stories-title"><header class="ui-stories-hero"><p class="eyebrow">FAMILY STORIES</p><h2 id="stories-title">Stories from the family record</h2><p>Source-controlled highlights grouped by era. Provisional and unresolved events keep their evidence qualification visible, while rejected research stays in the Research Center rather than the family story.</p></header>${chapters.length?chapters.map(([decade,items])=>`<section class="ui-story-chapter"><div class="ui-story-chapter-heading"><span>${esc(decade)}</span><h3>${items.length} recorded ${items.length===1?'event':'events'}</h3></div><div class="ui-story-list">${items.slice(0,8).map(card).join('')}</div></section>`).join(''):'<div class="empty">No source-controlled story events are available yet.</div>'}</section>`;
}
