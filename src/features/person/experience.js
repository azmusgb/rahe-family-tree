...
function momentCard(event){
  ...
  return`<article class="ui-person-story-moment"><time>${esc(eventDate(event))}</time><div><span>${esc(eventType(event))}</span><h3>${esc(eventText(event))}</h3>${place?`<p>${esc(place)}</p>`:''}</div></article>`;
}
function storyMarkup(person){
  const moments=supportedMoments(person);
  if(person.living)return`<section id="v17-story" class="ui-person-story living" aria-labelledby="v173-story-title"><div class="ui-story-copy"><p class="eyebrow">LIFE STORY</p><h2 id="v173-story-title">Part of the living family</h2><p>This public profile keeps private chronology and location details protected while preserving documented family connections.</p></div></section>`;
  if(!moments.length)return`<section id="v17-story" class="ui-person-story quiet" aria-labelledby="v173-story-title"><div class="ui-story-copy"><p class="eyebrow">LIFE STORY</p><h2 id="v173-story-title">A place in the family story</h2><p>No supported public-safe life moments are currently linked closely enough to feature here. Family connections and qualified chronology remain available below.</p></div></section>`;
  return`<section id="v17-story" class="ui-person-story" aria-labelledby="v173-story-title"><div class="ui-story-copy"><p class="eyebrow">LIFE STORY</p><h2 id="v173-story-title">A life in the family record</h2><p>Selected supported moments from the source-controlled archive introduce this person before the full chronology and research detail.</p></div><div class="ui-person-story-moments">${moments.map(momentCard).join('')}</div></section>`;
}
function makeResearchProgressive(root){
  const research=root.querySelector('#v17-research');
  if(!research||research.querySelector('.ui-research-details'))return;
  ...
  details.className='ui-research-details';
  ...
  const body=document.createElement('div');body.className='ui-research-body';
...
function tuneLifeSection(root,person){
  ...
  const title=life.querySelector('.ui-section-head h2');
  const intro=life.querySelector('.ui-section-head p:not(.eyebrow)');
...
function addStoryNav(root){
  const nav=root.querySelector('.ui-person-nav');if(!nav||nav.querySelector('[href="#v17-story"]'))return nav;
...
function enhance(){
  ...
  const root=document.querySelector(`.ui-person[data-person-id="${cssEscape(id)}"]`),person=personById(id);
