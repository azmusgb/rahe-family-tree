import{familyPrimary,familyExplore,researchPrimary,familyLabels,researchRoutes,owningSection,linksHtml}from'./model.js';
import{routeKeyFromLocation,routeDetailFromLocation}from'./runtime.js';
...
function familyMenus(){return `<details class="nav-menu explore-menu nav-menu v158-explore"><summary aria-expanded="false">Explore</summary><div class="nav-popover">${linksHtml(familyExplore)}</div></details><a class="research-entry ui-research-entry" href="#research">Research</a>`;}
function researchPrimaryHtml(){return linksHtml(researchPrimary);}
function researchMenus(){return `<a class="family-return ui-family-return" href="#dashboard" data-family-return data-ui-family-return>← Back to Family</a>`;}

function preserveActions(menus){return menus?.querySelector('.page-actions--desktop,.ui-desktop-actions')||null;}
...
function ensureDesktopContainers(){
  ...
  let primary=nav.querySelector('.primary-nav'),menus=nav.querySelector('.nav-menus');
  if(!primary||!menus){
    rememberPersistentLinks(nav);
    const actions=preserveActions(nav);
    primary=document.createElement('div');primary.className='primary-nav';
    menus=document.createElement('div');menus.className='nav-menus';
...
function contextualSearch(route=routeKey()){
  ...
  label.childNodes[0].nodeValue=text;input.placeholder=placeholder;routeShell?.classList.toggle('ui-context-search',family&&!hideFamilyFilters);
}
...
function returnToFamily(event){const trigger=event.target.closest?.('[data-family-return],[data-ui-family-return],[data-v158-family-mobile]');if(!trigger)return false;if(document.body.dataset.experience==='research')document.querySelector('.experience-toggle')?.click();location.hash='dashboard';return true;}
...
function syncHeaderContext(route=routeKey()){const header=document.querySelector('.site-header');if(header)header.dataset.context=isResearchContext(route)?'research':'family';document.querySelectorAll('.primary-nav').forEach(el=>el.classList.add('primary-nav'));document.querySelectorAll('.nav-menus').forEach(el=>el.classList.add('nav-menus'));document.querySelectorAll('.nav-menu').forEach(el=>el.classList.add('nav-menu'));document.querySelectorAll('.nav-popover').forEach(el=>el.classList.add('nav-popover'));document.querySelectorAll('.ui-research-entry').forEach(el=>el.classList.add('research-entry'));document.querySelectorAll('.ui-family-return').forEach(el=>el.classList.add('family-return'));document.querySelectorAll('.mobile-more').forEach(el=>el.classList.add('mobile-more'));document.querySelector('.ui-profile-nav')?.classList.add('profile-nav');document.querySelectorAll('.nav-menu[open],.mobile-more[open]').forEach(details=>{if(details.dataset.keepOpen!=='true')details.removeAttribute('open');});document.querySelectorAll('.nav-menu,.mobile-more,.site-tools,.tools-menu').forEach(details=>details.querySelector(':scope > summary')?.setAttribute('aria-expanded',String(details.open)));}
...
function previewRoute(route){if(!route||isResearchContext(route)!==isResearchContext())return;const current=owningSection(route);markCurrent(document.querySelector('.primary-nav'),'[data-nav-key],a[href^="#"]',current);markCurrent(document.getElementById('family-mobile-dock'),'[data-dock-route],a[href^="#"]',current);document.body.dataset.navigationShellRoute=route;}
...
document.addEventListener('toggle',event=>{const details=event.target;if(!(details instanceof HTMLDetailsElement)||!details.matches('.mobile-more,.nav-menu,.nav-menu,.site-tools,.tools-menu'))return;details.querySelector(':scope > summary')?.setAttribute('aria-expanded',String(details.open));if(details.open)closeTransientNavigation(details);},true);
