...
function miniPerson(person,label){
  if(!person)return'';
  return`<button type="button" class="ui-dashboard-tree-person" data-person="${esc(person.id)}"><span aria-hidden="true">${esc(initials(person.name))}</span><small>${esc(label)}</small><b>${esc(person.name.replace(/\s*\/.*$/,''))}</b></button>`;
}
...
function enrichPeople(content){
  content.querySelectorAll('.dashboard-person-open[data-person]').forEach(button=>{
    ...
    button.closest('.dashboard-person')?.classList.add('ui-dashboard-person-card');
    let copy=button.querySelector('.ui-person-copy');
    if(!copy){copy=document.createElement('span');copy.className='ui-person-copy';button.appendChild(copy);}
...
function moveSections(content){
  ...
  let tree=content.querySelector('.ui-tree-preview');if(!tree){hero.insertAdjacentHTML('afterend',buildTreePreview());tree=content.querySelector('.ui-tree-preview');}
...
function installResearchCenter(content){
  ...
  let center=content.querySelector('.ui-research-center');if(center)return;
  center=document.createElement('section');center.className='ui-research-center';
...
function installMediaPreview(content,media){
  ...
  section.innerHTML=`<div class="section-title"><div><p class="eyebrow">FAMILY PHOTOS</p><h2>Faces from the archive</h2></div><a href="#media">Browse all media ↗</a></div><div class="v157-media-grid">${images.map(m=>`<a href="#media" class="ui-media-card"><img src="/api/media?file=${encodeURIComponent(m.id)}" alt="${esc(m.caption||m.title||'Family photograph')}" loading="lazy" decoding="async"><span><b>${esc(m.title||'Family photograph')}</b><small>${esc([m.eventDate,typeof m.location==='string'?m.location:''].filter(Boolean).join(' · ')||'Family archive')}</small></span></a>`).join('')}</div>`;
}
