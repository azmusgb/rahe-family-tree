...
function removeFallbackContext(){document.querySelector('.tree-context-fallback')?.remove();document.querySelector('.tree-recent-history--fallback')?.remove();}
function installFallbackContext(){
  if(document.querySelector('.tree-advanced-nav')){removeFallbackContext();return;}
  ...
  if(!nav){nav=document.createElement('nav');nav.className='tree-context-nav tree-context-fallback tree-context-breadcrumb ui-breadcrumb';nav.setAttribute('aria-label','Tree context');shell.insertAdjacentElement('beforebegin',nav);}
  ...
  const markup=parts.join('');if(nav.dataset.signature!==markup){nav.innerHTML=markup;nav.dataset.signature=markup;}

  let history=document.querySelector('.tree-recent-history--fallback');
  if(!recent.length){history?.remove();return;}
  if(!history){history=document.createElement('div');history.className='tree-recent-history tree-recent-history--fallback';history.setAttribute('aria-label','Recently viewed people');nav.insertAdjacentElement('afterend',history);}
...
function annotateCouples(){document.querySelectorAll('.edge.couple-child').forEach(edge=>edge.classList.add('tree-edge-couple-child','v1291-couple-child'));document.querySelectorAll('.edge.spouse,.edge.spouse-lead').forEach(edge=>edge.classList.add('tree-edge-spouse','ui-spouse'));}
...
function installMobileHint(){if(route()!=='tree')return;const scroll=document.querySelector('.graph-scroll');if(!scroll||document.querySelector('.ui-mobile-hint'))return;const hint=document.createElement('div');hint.className='tree-mobile-hint ui-mobile-hint';hint.textContent='Tap a person to open their profile. Use the tree controls to choose a family view, change generations, or move to another connected family.';scroll.insertAdjacentElement('beforebegin',hint);}
function syncRelease(){document.body.dataset.treePolish='active';document.body.dataset.treePolishRelease='18.6';}
function apply(){syncRelease();if(route()!=='tree'){removeFallbackContext();return;}installFallbackContext();annotateCouples();improveCollapse();installMobileHint();const focus=focusId();if(focus&&matchMedia('(max-width:760px)').matches&&document.body.dataset.lastCentered!==focus){document.body.dataset.lastCentered=focus;requestAnimationFrame(()=>center(focus,true));}}
...
window.addEventListener('hashchange',()=>setTimeout(apply));window.addEventListener('family-media-changed',()=>setTimeout(apply));window.addEventListener('family-view-rendered',()=>setTimeout(apply));const observer=new MutationObserver(()=>queueMicrotask(apply));const start=()=>{const c=document.querySelector('#content');if(c)observer.observe(c,{childList:true,subtree:false});apply();};document.readyState==='loading'?document.addEventListener('DOMContentLoaded',start):start();
