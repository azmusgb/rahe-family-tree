...
function installBranchContext(content){if(routeKey()!=='people')return;const selected=String(document.getElementById('branch')?.value||'').trim(),existing=content.querySelector('.v162-branch-context');if(!selected){existing?.remove();return;}const profile=branchProfile(selected),grid=content.querySelector('.people-grid');if(!grid)return;const years=profile.years.length?`${Math.min(...profile.years)}–${Math.max(...profile.years)}`:'Dates vary by person';const html=`<div><p class="eyebrow">${esc(selected.toUpperCase())} FAMILY</p><h3>${profile.people.length} ${profile.people.length===1?'person':'people'} in this branch</h3><p>${esc(years)}${profile.places.length?` · ${esc(profile.places.join(' · '))}`:''}</p></div><div class="v162-branch-actions"><a href="#stories">Stories</a><a href="#migration">Places</a></div>`;if(existing){existing.innerHTML=html;return;}const section=document.createElement('section');section.className='v162-branch-context';section.innerHTML=html;grid.insertAdjacentElement('beforebegin',section);}
