...
function familyMoments(){return events().filter(e=>stateOf(e)==='SUPPORTED'&&yearOf(e)&&eventText(e)).sort((a,b)=>yearOf(b)-yearOf(a));}
...
function topPlaces(limit=5){const counts=new Map();for(const event of events().filter(e=>stateOf(e)==='SUPPORTED'))for(const place of eventPlaces(event))counts.set(place,(counts.get(place)||0)+1);return[...counts].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0])).slice(0,limit).map(([place])=>place);}
...
function branchProfile(branch){const people=displayPeople().filter(p=>cleanBranch(p.branch)===branch),ids=new Set(people.map(p=>p.id)),places=new Map(),years=[];for(const p of people)for(const y of String(p.dates||'').match(/\b(?:1[6-9]\d{2}|20\d{2})\b/g)||[])years.push(Number(y));for(const event of events()){if(!(event.personIds||[]).some(id=>ids.has(id)))continue;for(const place of eventPlaces(event))places.set(place,(places.get(place)||0)+1);}return{people,years:years.filter(Number.isFinite),places:[...places].sort((a,b)=>b[1]-a[1]).slice(0,3).map(([place])=>place)};}
