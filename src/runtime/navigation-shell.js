...
function rebuildDesktopNav(){
  ...
  document.body.dataset.v158Context=context;
...
function openGlobalSearch(){document.querySelector('.v158-mobile-more')?.removeAttribute('open');if(focusSearchInput())return;if(isResearchContext()){location.hash='research';setTimeout(focusSearchInput,180);return;}location.hash='people';let tries=0;const focus=()=>{if(focusSearchInput()||tries++>8)return;requestAnimationFrame(focus);};requestAnimationFrame(focus);}
function syncHeaderContext(){const header=document.querySelector('.site-header');if(header)header.dataset.context=isResearchContext()?'research':'family';document.querySelectorAll('.v151-nav-menu[open],.v158-mobile-more[open]').forEach(details=>{if(details.dataset.keepOpen!=='true')details.removeAttribute('open');});}
