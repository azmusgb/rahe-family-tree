// v17.5 Elevated Family Experience — premium family-first presentation helpers.
// Presentation only: reads the canonical model without mutating genealogy,
// evidence states, source IDs, relationship semantics, or privacy controls.
...
function installHomeDiscovery(root){
  if(routeKey()!=='dashboard'||root.querySelector('.v175-discovery'))return;
  const hero=root.querySelector('.v17-home-hero');if(!hero)return;
  const people=recentPeople(4);if(!people.length)return;
  const section=document.createElement('section');section.className='v175-discovery';
  section.innerHTML=`<div class="v175-discovery-head"><div><p class="eyebrow">KEEP EXPLORING</p><h2>Follow another path through the family</h2><p>Open a person, then move through their relatives, life story, places, photographs, and records.</p></div><a href="#people">Browse everyone ↗</a></div><div class="v175-discovery-grid">${people.map(person=>`<button type="button" data-person="${esc(person.id)}"><span class="v175-discovery-avatar" data-v17-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><span><small>${esc(cleanBranch(person))}</small><b>${esc(cleanName(person.name))}</b><em>${person.living?'Living — details protected':esc(person.dates||'Historical family record')}</em></span><i aria-hidden="true">→</i></button>`).join('')}</div>`;
...
function installPersonSnapshot(root,person){
  if(!person||root.querySelector('.v175-profile-snapshot'))return;
  ...
  const section=document.createElement('section');section.className='v175-profile-snapshot';section.setAttribute('aria-label','Profile summary');
  section.innerHTML=`<div class="v175-snapshot-intro"><span class="eyebrow">AT A GLANCE</span><b>${esc(cleanBranch(person))} family</b></div><div class="v175-snapshot-stat"><b>${family.length}</b><span>close family connection${family.length===1?'':'s'}</span></div><div class="v175-snapshot-stat"><b>${person.living?'Protected':events.length}</b><span>${person.living?'private chronology':'linked life record'+(events.length===1?'':'s')}</span></div><div class="v175-snapshot-stat"><b>${person.living?'Protected':places.length}</b><span>${person.living?'private places':'recorded place'+(places.length===1?'':'s')}</span></div>`;
  (nav||header).insertAdjacentElement(nav?'beforebegin':'afterend',section);
}
function installActivePersonNav(root){
  const nav=root.querySelector('.v17-person-nav');if(!nav||nav.dataset.v175ActiveNav==='ready')return;
  nav.dataset.v175ActiveNav='ready';
  ...
  const observer=new IntersectionObserver(entries=>{const visible=entries.filter(entry=>entry.isIntersecting).sort((a,b)=>Math.abs(a.boundingClientRect.top)-Math.abs(b.boundingClientRect.top))[0];if(visible)setActive(visible.target);},{rootMargin:'-28% 0px -58% 0px',threshold:[0,.1,.5]});
  sections.forEach(row=>observer.observe(row.target));
}
function elevatePerson(){
  if(routeKey()!=='person')return;
  const id=routePersonId(),person=personById(id),escaped=globalThis.CSS?.escape?CSS.escape(id):id.replace(/[^a-zA-Z0-9_-]/g,'');
  const root=document.querySelector(`.v17-person[data-person-id="${escaped}"]`);if(!root||!person)return;
...
function treeFocusPerson(){const url=new URL(location.href),id=url.searchParams.get('focus')||document.querySelector('[data-tree-person]')?.value||'';return personById(id)||null;}
function scopeCopy(scope){return({family:['Close family','Parents, spouses, children, and nearby generations around the focal person.'],ancestors:['Ancestors','Follow the focal person backward through documented parent lines.'],descendants:['Descendants','Follow the focal person forward through documented child lines.'],connected:['Connected family','Explore the broader connected family network across branches.'],all:['Full graph','Show every active public-safe relationship in the current graph.']})[scope]||['Family view','Explore documented family relationships.'];}
function installTreeContext(root){
  if(routeKey()!=='tree')return;
  const graph=root.querySelector('.graph-shell');if(!graph)return;
  let panel=root.querySelector('.v175-tree-context');if(!panel){panel=document.createElement('section');panel.className='v175-tree-context';graph.insertAdjacentElement('beforebegin',panel);}
  const scope=treeScope(),person=treeFocusPerson(),copy=scopeCopy(scope);
  panel.innerHTML=`<div class="v175-tree-context-copy"><span class="eyebrow">TREE VIEW</span><b>${esc(copy[0])}${person?` · ${esc(cleanName(person.name))}`:''}</b><p>${esc(copy[1])}</p></div><div class="v175-tree-scope" role="group" aria-label="Tree view"><button type="button" data-tree-scope="family" class="${scope==='family'?'active':''}" aria-pressed="${scope==='family'}">Family</button><button type="button" data-tree-scope="ancestors" class="${scope==='ancestors'?'active':''}" aria-pressed="${scope==='ancestors'}">Ancestors</button><button type="button" data-tree-scope="descendants" class="${scope==='descendants'?'active':''}" aria-pressed="${scope==='descendants'}">Descendants</button><button type="button" data-tree-scope="connected" class="${scope==='connected'?'active':''}" aria-pressed="${scope==='connected'}">Connected</button></div>`;
}

function apply(){if(!isFamily())return;const root=document.querySelector('#content [data-v17-native]');if(!root)return;if(routeKey()==='dashboard')installHomeDiscovery(root);if(routeKey()==='person')elevatePerson();if(routeKey()==='tree')installTreeContext(root);}
