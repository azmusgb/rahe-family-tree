...
const unique=rows=>[...new Map(rows.filter(Boolean).map(row=>[row.id,row])).values()];
...
function personCard(person){return`<article class="v175-branch-person"><button type="button" data-person="${esc(person.id)}"><span class="v17-avatar" data-v17-person-photo="${person.living?'':esc(person.id)}">${esc(initials(person.name))}</span><span><small>${esc(personBranches(person).join(' · '))}</small><b>${esc(cleanName(person.name))}</b><em>${esc(publicDates(person))}</em><p>${esc(String(person.role||'Family member'))}</p></span><i aria-hidden="true">→</i></button></article>`;}
...
function apply(){promoteHomeBranchLinks();updatePeopleDirectoryLink();}
