...
function installHomeBranchIndex(root){
  ...
  const section=document.createElement('section');section.className='v172-home-branches';section.innerHTML=`<div class="v17-section-head"><div><p class="eyebrow">FAMILY BRANCHES</p><h2>One archive, many connected lines</h2><p>Every documented branch is a peer part of the family history. Open a branch for its people, places, stories, and records.</p></div><a href="#families">Explore all ${branches.length} branches ↗</a></div><div class="v172-branch-grid">${cards.slice(0,6).join('')}</div>${cards.length>6?`<details class="home-branch-disclosure" data-keep-open="true"><summary>More family branches (${cards.length-6})</summary><div class="v172-branch-grid">${cards.slice(6).join('')}</div></details>`:''}`;hero.insertAdjacentElement('afterend',section);
}
