...
function peopleCard(person){return`<article class="person-card"><button type="button" data-person="${esc(person.id)}"><span class="v17-avatar" data-v17-person-photo="${esc(person.id)}">${esc(initials(person.name))}</span><span class="v17-person-card-copy"><small>${esc(personBranch(person))}</small><b>${esc(cleanName(person.name))}</b><em>${esc(publicDates(person))}</em><p>${esc(relationshipLine(person))}</p></span><i aria-hidden="true">→</i></button></article>`;}
