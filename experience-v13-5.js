...
function recordCurrentPerson(){if(routeKey()!=='person')return;const id=routeId(),p=personById(id);if(!p)return;writeRecent([id,...readRecent().filter(x=>x!==id)]);}
...
async function enhanceMediaMetric(){
  if(routeKey()!=='dashboard')return;const metrics=document.querySelector('.dashboard-family-metrics');if(!metrics)return;
  const data=await getMediaSummary();if(!data||routeKey()!=='dashboard'||!document.body.contains(metrics))return;
