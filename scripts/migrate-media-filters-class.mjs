...
const legacy='v161-media-filters';
const semantic='media-filters';
...
const excludedFiles=new Set(['scripts/migrate-media-filters-class.mjs']);
...
for(const full of walk(root)){
  ...
  const next=original.replace(exact,semantic)
    .replace(/\bmedia-filters\s+media-filters\b/g,'media-filters')
    .replace(/\.media-filters\s*,\s*\.media-filters(?![-_a-zA-Z0-9])/g,'.media-filters');
