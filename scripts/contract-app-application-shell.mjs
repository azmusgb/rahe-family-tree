...
const familyBoundary=fs.readFileSync('src/features/family/index.js','utf8');
const treeBoundary=fs.readFileSync('src/features/tree/index.js','utf8');
const treeController=fs.readFileSync('src/features/tree/controller.js','utf8');
const mediaBoundary=fs.readFileSync('src/features/media/index.js','utf8');
...
const experienceRuntime=fs.readFileSync('src/app/experience-core.js','utf8');
...
const treeRuntime=fs.readFileSync('src/features/tree/engine.js','utf8');
const treePolish=fs.readFileSync('src/features/tree/polish.js','utf8');
const dashboard=fs.readFileSync('dashboard-v13-3.js','utf8');
const mediaPage=fs.readFileSync('src/features/media/page.js','utf8');
...
test('current browser runtime uses one cache-busted production entry with canonical app composition',()=>{const version=releaseOf(index,/data-ui-release="(\d+\.\d+\.\d+)"/),scripts=[...index.matchAll(/<script type="module" src="([^"]+)"/g)].map(m=>m[1]);assert.deepEqual(scripts,[`app.bundle.js?v=${version}`]);assert.match(entry,/src\/app\/runtime\.js/);assert.doesNotMatch(entry,/src\/runtime\/index\.js/);const expected=['../../src/app/router.js','../runtime/base-controls.js','../../media.js','../../deployment.js','../features/family/index.js','../features/media/index.js','../features/tree/index.js','../features/search/index.js','../../src/features/media/page.js','../runtime/experience.js'];const actual=[...appRuntime.matchAll(/import ['"]([^'"]+\.js)['"]/g)].map(match=>match[1]);assert.deepEqual(actual,expected);assert.doesNotMatch(entry,/search-v13-2\.js/);assert.doesNotMatch(entry,/v15-runtime\.js/);const eager=['experience-core','../../v15-family-focus','../../platform-v13-runtime','page-architecture','navigation-shell','performance-contracts','native-family-v17-controller','tree-controller','mobile-ui-ownership','mobile-experience','mobile-route-state','mobile-ui-shell','mobile-ui-transient','mobile-ui-phase7'];const experienceImports=[...experience.matchAll(/import ['"]([^'"]+)\.js['"]/g)].map(match=>match[1].replace(/^\.\//,''));assert.deepEqual(experienceImports,eager);assert.doesNotMatch(experience,/v15-1-runtime\.js/);assert.match(routeCapabilities,/name:'unified-family-experience'[\s\S]*routes:\['dashboard','tree'\]/);assert.doesNotMatch(experience,/import\('\.\/unified-family-experience\.js'\)/);assert.match(treeController,/v17-6-stability\.js/);assert.match(treeController,/tree-advanced\.js/);});

test('family semantic boundary preserves historical initialization order',()=>{const expected=['../../runtime/family-mode.js','../../runtime/family-profile.js','../../runtime/family-qa.js','../../runtime/family-contributions.js'];const actual=[...familyBoundary.matchAll(/import ['"]([^'"]+\.js)['"]/g)].map(match=>match[1]);assert.deepEqual(actual,expected);for(const retired of['v12-6.js','v12-6-1.js','v12-6-2.js','v12-7.js'])assert.equal(fs.existsSync(retired),false);});

test('tree semantic boundary preserves historical initialization order',()=>{const expected=['../../runtime/tree-engine.js','../../runtime/tree-polish.js'];const actual=[...treeBoundary.matchAll(/import ['"]([^'"]+\.js)['"]/g)].map(match=>match[1]);assert.deepEqual(actual,expected);for(const retired of['v12-9.js','v12-9-1.js'])assert.equal(fs.existsSync(retired),false);});

test('media semantic boundary owns the v12.8 enhancement layer',()=>{assert.match(mediaBoundary,/import '\.\.\/\.\.\/runtime\/media-enhancements\.js'/);assert.equal(fs.existsSync('v12-8.js'),false);});

test('production build splits JavaScript and preserves semantic CSS with no compatibility layer',()=>{assert.match(build,/ESBUILD_VERSION='0\.25\.10'/);assert.match(build,/app-entry\.js/);assert.match(build,/--bundle/);assert.match(build,/--splitting/);assert.match(build,/--outdir=dist/);assert.match(build,/--entry-names=app\.bundle/);assert.match(build,/--chunk-names=chunks\/\[name\]-\[hash\]/);assert.match(build,/src\/styles\/index\.css/);assert.doesNotMatch(styleRoot,/legacy-compat\.generated\.css/);assert.match(build,/const legacyStyleSources=\[\]/);assert.match(build,/compatibilityBoundary:null/);assert.match(build,/legacySourceCount:0/);for(const sheet of['v14.css','v15.css','v15-1.css','v15-5.css','v15-6.css','v15-8.css'])assert.equal(fs.existsSync(sheet),false);for(const semantic of['tokens.css','core.css','composition.css','experience.css','home-responsive.css','interaction.css','mobile.css','print.css'])assert.match(styleRoot,new RegExp(`@import './${semantic.replace('.','\\.')}'`));assert.match(build,/outfile=dist\/styles\.css/);assert.match(build,/browserAssets:\[\.\.\.jsAssets,'styles\.css'\]/);assert.match(build,/splitting:true/);assert.match(build,/chunkDirectory:'chunks'/);});
