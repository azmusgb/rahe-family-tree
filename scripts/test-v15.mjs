...
test('mobile tree becomes canvas-first without changing graph semantics',()=>{assert.match(mobileTree,/v1511-mobile-tree/);assert.match(mobileTree,/Focus person/);assert.match(mobileTree,/Full tree/);assert.match(mobileTree,/v1511-tree-count/);assert.match(mobileTree,/v1511-tree-options/);assert.match(mobileTree,/options\.append\(focusbar\)/);assert.match(mobileTree,/options\.insertAdjacentElement\('afterend',shell\)/);assert.match(mobileTree,/v154-tree-person/);assert.doesNotMatch(mobileTree,/\.state\s*=\s*[^=]/);assert.match(styleRoot,/@import '\.\/mobile-tree-experience\.css'/);assert.match(mobileTreeCss,/Mobile tree is canvas-first/);assert.match(mobileTreeCss,/\.v1511-tree-options-summary/);assert.match(mobileTreeCss,/\.v154-tree-person/);assert.match(mobileTreeCss,/\.v129-tree-memory/);assert.match(mobileTreeCss,/\.mobile-family-list/);assert.match(mobileTreeCss,/\.relationship-index/);assert.match(mobileTreeCss,/height:calc\(100dvh - 178px\)/);assert.match(mobileTreeCss,/min-height:430px/);assert.match(mobileTreeCss,/z-index:1000!important/);});
...
test('mobile dock is a real touch surface above application content',()=>{assert.match(css,/#family-mobile-dock:not\(\[hidden\]\)/);assert.match(css,/z-index:2147483000!important/);assert.match(css,/pointer-events:auto!important/);assert.match(css,/touch-action:manipulation/);assert.match(css,/--tap-target:48px/);assert.match(overhaulCss,/z-index:1000!important/);});
