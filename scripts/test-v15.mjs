...
const mobileTree=fs.readFileSync('src/runtime/mobile-tree-experience.js','utf8');
const styleRoot=fs.readFileSync('src/styles/index.css','utf8');
const familySearchCss=fs.readFileSync('src/styles/family-search-experience.css','utf8');
const mobileTreeCss=fs.readFileSync('src/styles/mobile-tree-experience.css','utf8');
...
test('experience boundary preserves initialization order through v15.11 mobile refinements',()=>{const expected=['./experience-core.js','../../v15-1-runtime.js','../../v15-family-focus.js','../../platform-v13-runtime.js','./page-architecture.js','./home-flow.js','./navigation-shell.js','./people-person-experience.js','./compact-disclosure.js','./mobile-family-overhaul.js','./mobile-tree-experience.js'];const actual=[...experience.matchAll(/import ['"]([^'"]+\.js)['"]/g)].map(match=>match[1]);assert.deepEqual(actual,expected);});
...
test('v15.8 simplifies family navigation and separates the research center shell',()=>{assert.match(navShell,/Home<\/a><a href="#tree"[^>]*>Tree<\/a><a href="#people"[^>]*>People<\/a><a href="#media"[^>]*>Photos<\/a>/);assert.match(navShell,/Timeline/);assert.match(navShell,/Places & Migration/);assert.match(navShell,/Stories/);assert.match(navShell,/Research Center/);assert.match(navShell,/RESEARCH CENTER/);assert.match(navShell,/Find someone in the family/);assert.match(navShell,/data-dock-route="media"[^>]*><span>Photos<\/span>/);assert.match(navShell,/v158-mobile-more/);assert.match(navShell,/document\.body\.append\(dock\)/);assert.match(navCss,/body\[data-v158-context="research"\]/);assert.match(navCss,/Reduce card-on-card framing/);});

test('v15.9 makes People and Person family-first without changing genealogy state',()=>{assert.match(peoplePerson,/FAMILY DIRECTORY/);assert.match(peoplePerson,/Browse by branch/);assert.match(peoplePerson,/LIFE AT A GLANCE/);assert.match(peoplePerson,/Living — details protected/);assert.match(peoplePerson,/visibility==='public'/);assert.match(peoplePerson,/person\.living\)return\[\]/);assert.doesNotMatch(peoplePerson,/\.state\s*=\s*[^=]/);assert.match(peoplePersonCss,/People directory/);assert.match(peoplePersonCss,/Person profile becomes the primary family story/);assert.match(peoplePersonCss,/data-route="person"[^\n]*\.person-hero/);});
...
test('v15.11 composes home into one lead and turns search into a non-flow sheet',()=>{assert.match(overhaul,/v1511-home-lead/);assert.match(overhaul,/Meet the people, follow the branches/);assert.match(overhaul,/Your connected family/);assert.match(overhaul,/v1511-featured/);assert.match(overhaul,/v1511-search-sheet/);assert.match(overhaul,/family-search-scrim/);assert.match(overhaul,/ensureSearchPortal/);assert.match(overhaul,/ensureCloseButton/);assert.doesNotMatch(overhaul,/\.state\s*=\s*[^=]/);assert.match(overhaulCss,/One composed lead/);assert.match(overhaulCss,/dashboard-person:nth-child\(n\+5\)/);assert.match(overhaulCss,/dashboard-hero-card\[hidden\]/);assert.match(styleRoot,/\.\/family-search-experience\.css/);assert.match(familySearchCss,/Family search experience/);assert.match(familySearchCss,/family-search-command/);assert.match(familySearchCss,/position:fixed!important/);assert.match(familySearchCss,/z-index:2001!important/);assert.match(familySearchCss,/family-search-scrim/);assert.match(familySearchCss,/pointer-events:none!important/);assert.match(familySearchCss,/grid-template-rows:auto auto minmax\(0,1fr\) auto/);assert.match(familySearchCss,/max-height:calc\(100dvh - 76px/);});

test('mobile tree becomes canvas-first without changing graph semantics',()=>{assert.match(mobileTree,/v1511-mobile-tree/);assert.match(mobileTree,/Focus person/);assert.match(mobileTree,/Full tree/);assert.match(mobileTree,/v1511-tree-count/);assert.match(mobileTree,/v1511-tree-options/);assert.match(mobileTree,/options\.append\(focusbar\)/);assert.match(mobileTree,/options\.insertAdjacentElement\('afterend',shell\)/);assert.match(mobileTree,/v154-tree-person/);assert.doesNotMatch(mobileTree,/\.state\s*=\s*[^=]/);assert.match(styleRoot,/@import '\.\/mobile-tree-experience\.css'/);assert.match(mobileTreeCss,/Mobile tree is canvas-first/);assert.match(mobileTreeCss,/\.v1511-tree-options-summary/);assert.match(mobileTreeCss,/\.v154-tree-person/);assert.match(mobileTreeCss,/\.v129-tree-memory/);assert.match(mobileTreeCss,/\.mobile-family-list/);assert.match(mobileTreeCss,/\.relationship-index/);assert.match(mobileTreeCss,/height:calc\(100dvh - 178px\)/);assert.match(mobileTreeCss,/min-height:430px/);assert.match(mobileTreeCss,/z-index:2147483000!important/);});

test('semantic stylesheet boundary preserves the deployed cascade order',()=>{const expected=['v11.css','v11-nav.css','v11-2.css','v11-3.css','v11-4.css','v11-6.css','v12.css','v12-2.css','v12-3.css','v12-4.css','v12-5.css','v12-6.css','v12-6-1.css','v12-6-2.css','v12-7.css','v12-8.css','v12-9.css','v12-9-1.css','v13-0.css','dashboard-v13-2.css','media-page-v13-4.css','experience-v13-5.css','v14.css','v15.css','v15-1.css','v15-family-focus.css','platform-v13.css','v15-5.css','v15-6.css','v15-7.css','v15-8.css','v15-9.css','v15-10.css','v15-11.css'];const actual=[...styleRoot.matchAll(/@import ['"]\.\.\/\.\.\/([^'"]+\.css)['"]/g)].map(match=>match[1]);assert.deepEqual(actual,expected);assert.match(styleRoot,/@import '\.\/family-search-experience\.css'/);assert.match(styleRoot,/@import '\.\/mobile-tree-experience\.css'/);assert.match(build,/src\/styles\/index\.css/);assert.doesNotMatch(build,/const cssSources=/);assert.doesNotMatch(build,/\.styles-v15\.source\.css/);});

test('mobile dock is a real touch surface above application content',()=>{assert.match(css,/#family-mobile-dock:not\(\[hidden\]\)/);assert.match(css,/z-index:2147483000!important/);assert.match(css,/pointer-events:auto!important/);assert.match(css,/touch-action:manipulation/);assert.match(css,/--tap-target:48px/);assert.match(overhaulCss,/z-index:2147483000!important/);});
