...
test('v15.11 composes home into one lead and turns search into a non-flow sheet',()=>{assert.match(overhaul,/v1511-home-lead/);assert.match(overhaul,/Meet the people, follow the branches/);assert.match(overhaul,/Your connected family/);assert.match(overhaul,/v1511-featured/);assert.match(overhaul,/v1511-search-sheet/);assert.match(overhaul,/family-search-scrim/);assert.match(overhaul,/ensureSearchPortal/);assert.match(overhaul,/ensureCloseButton/);assert.doesNotMatch(overhaul,/\.state\s*=\s*[^=]/);assert.match(overhaulCss,/One composed lead/);assert.match(overhaulCss,/dashboard-person:nth-child\(n\+5\)/);assert.match(overhaulCss,/dashboard-hero-card\[hidden\]/);assert.match(styleRoot,/\.\/family-search-experience\.css/);assert.match(familySearchCss,/Family search experience/);assert.match(familySearchCss,/family-search-command/);assert.match(familySearchCss,/position:fixed!important/);assert.match(familySearchCss,/z-index:2147483646!important/);assert.match(familySearchCss,/family-search-scrim/);assert.match(familySearchCss,/grid-template-rows:auto auto minmax\(0,1fr\) auto/);assert.match(familySearchCss,/max-height:calc\(100dvh - 76px/);});

test('mobile tree becomes canvas-first without changing graph semantics',()=>{assert.match(mobileTree,/v1511-mobile-tree/);assert.match(mobileTree,/Focus person/);assert.match(mobileTree,/Full tree/);assert.match(mobileTree,/v1511-tree-count/);assert.match(mobileTree,/v154-tree-person/);assert.doesNotMatch(mobileTree,/\.state\s*=\s*[^=]/);assert.match(styleRoot,/@import '\.\/mobile-tree-experience\.css'/);assert.match(mobileTreeCss,/Mobile tree is canvas-first/);assert.match(mobileTreeCss,/\.v154-tree-person/);assert.match(mobileTreeCss,/\.v129-tree-memory/);assert.match(mobileTreeCss,/\.mobile-family-list/);assert.match(mobileTreeCss,/\.relationship-index/);assert.match(mobileTreeCss,/height:calc\(100dvh - 250px\)/);assert.match(mobileTreeCss,/min-height:390px/);});
