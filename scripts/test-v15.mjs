...
test('experience boundary cuts Home People Person and Tree into native v17 ownership',()=>{const expected=['./experience-core.js','../../v15-1-runtime.js','../../v15-family-focus.js','../../platform-v13-runtime.js','./page-architecture.js','./navigation-shell.js','./native-family-v17-controller.js'];const actual=[...experience.matchAll(/import ['"]([^'"]+)\.js['"]/g)].map(match=>match[1]);assert.deepEqual(actual,expected);assert.doesNotMatch(experience,/home-flow\.js/);assert.doesNotMatch(experience,/people-person-experience\.js/);assert.doesNotMatch(experience,/compact-disclosure\.js/);assert.match(experience,/unified-family-experience\.js/);assert.match(nativeController,/nativeRoutes=new Set\(\['dashboard','tree','people','person'\]\)/);for(const marker of['data-v17-native="home"','data-v17-native="people"','data-v17-native="person"'])assert.match(nativeRuntime,new RegExp(marker));assert.match(nativeController,/data-v17-native="tree"/);});
...
test('v17.2 experience runtime keeps dock navigation and freshness protection but drops legacy Home enhancers',()=>{assert.match(runtime,/#family-mobile-dock a\[href\^="#"\]/);assert.match(runtime,/event\.preventDefault\(\);navigate/);assert.match(runtime,/pageshow/);assert.match(runtime,/event\.persisted/);assert.match(runtime,/location\.reload\(\)/);assert.match(runtime,/build-info\.json\?ui-check=/);assert.match(runtime,/UI_RELEASE='17\.2\.0'/);assert.match(runtime,/STALE_RELOAD_KEY='family\.archive\.uiReload\.v17\.2'/);assert.doesNotMatch(runtime,/enhanceRecentPeople/);assert.doesNotMatch(runtime,/addDashboardPaths/);assert.doesNotMatch(runtime,/enhanceMediaMetric/);});
...
test('v17.2 production build emits one JavaScript bundle and one unversioned stylesheet asset name',()=>{assert.match(index,/app\.bundle\.js\?v=17\.2\.0/);assert.match(index,/styles\.css\?v=17\.2\.0/);assert.match(build,/const appVersion='17\.2\.0'/);assert.match(build,/outfile=dist\/app\.bundle\.js/);assert.match(build,/outfile=dist\/styles\.css/);assert.match(build,/bundler:`esbuild@\$\{ESBUILD_VERSION\}`/);assert.match(build,/browserAssets:\['app\.bundle\.js','styles\.css'\]/);});
