...
test('experience boundary cuts Home People Person and Tree into native v17 ownership',()=>{const expected=['./experience-core','../../v15-1-runtime','../../v15-family-focus','../../platform-v13-runtime','./page-architecture','./navigation-shell','./native-family-v17-controller'];const actual=[...experience.matchAll(/import ['"]([^'"]+)\.js['"]/g)].map(match=>match[1]);assert.deepEqual(actual,expected);assert.doesNotMatch(experience,/home-flow\.js/);assert.doesNotMatch(experience,/people-person-experience\.js/);assert.doesNotMatch(experience,/compact-disclosure\.js/);assert.match(experience,/unified-family-experience\.js/);assert.match(experience,/person-experience-v17-3\.js/);assert.match(nativeController,/nativeRoutes=new Set\(\['dashboard','tree','people','person'\]\)/);for(const marker of['data-v17-native="home"','data-v17-native="people"','data-v17-native="person"'])assert.match(nativeRuntime,new RegExp(marker));assert.match(nativeController,/data-v17-native="tree"/);});
...
test('current production build emits one JavaScript bundle and one stylesheet with v17.3 shell stamping',()=>{assert.match(index,/app\.bundle\.js\?v=17\.2\.0/);assert.match(index,/styles\.css\?v=17\.2\.0/);assert.match(build,/const appVersion='17\.3\.0'/);assert.match(build,/shell\.replaceAll\('17\.2\.0',appVersion\)/);assert.match(build,/outfile=dist\/app\.bundle\.js/);assert.match(build,/outfile=dist\/styles\.css/);assert.match(build,/bundler:`esbuild@\$\{ESBUILD_VERSION\}`/);assert.match(build,/browserAssets:\['app\.bundle\.js','styles\.css'\]/);});
