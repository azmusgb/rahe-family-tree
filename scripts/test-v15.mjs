...
test('mobile family navigation remains a persistent touch shell under the current release',()=>{assert.match(index,/id="family-mobile-dock"/);assert.match(index,/href="#dashboard"[^>]*data-dock-route="dashboard"/);assert.match(index,/href="#tree"[^>]*data-dock-route="tree"/);assert.match(index,/href="#families"[^>]*data-dock-route="families"/);assert.match(index,/href="#people"[^>]*data-dock-route="people"/);assert.match(index,/v158-mobile-more/);assert.match(index,/data-dock-search/);assert.match(index,/<a href="#media">Photos<\/a>/);assert.match(index,/app\.bundle\.js\?v=17\.5\.0/);assert.doesNotMatch(entry,/v15-runtime\.js/);assert.match(runtimeIndex,/import '\.\/experience\.js'/);});
...
test('experience boundary gives native v17 ownership to family routes including Families and Branch',()=>{const expected=['./experience-core','../../v15-1-runtime','../../v15-family-focus','../../platform-v13-runtime','./page-architecture','./navigation-shell','./native-family-v17-controller'];const actual=[...experience.matchAll(/import ['"]([^'"]+)\.js['"]/g)].map(match=>match[1]);assert.deepEqual(actual,expected);assert.doesNotMatch(experience,/home-flow\.js/);assert.doesNotMatch(experience,/people-person-experience\.js/);assert.doesNotMatch(experience,/compact-disclosure\.js/);assert.match(experience,/unified-family-experience\.js/);assert.match(experience,/person-experience-v17-3\.js/);assert.match(experience,/family-branches-v17-5\.js/);assert.match(nativeController,/nativeRoutes=new Set\(\['dashboard','tree','people','person','families','branch'\]\)/);for(const marker of['data-v17-native="home"','data-v17-native="people"','data-v17-native="person"'])assert.match(nativeRuntime,new RegExp(marker));assert.match(nativeController,/data-v17-native="tree"/);assert.match(nativeController,/renderFamiliesIndex\(\)/);assert.match(nativeController,/renderFamilyBranch\(routeBranchName\(\)\)/);});

test('v15.8 navigation semantics remain the shared family and research shell',()=>{for(const token of["label:'Home'","label:'Tree'","label:'People'","label:'Families'","label:'Photos'","label:'Timeline'","label:'Places & Migration'","label:'Stories'"])assert.match(navModel,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));assert.match(navShell,/RESEARCH CENTER/);assert.match(navShell,/Search people and branches/);assert.match(navShell,/Search the research archive/);assert.match(navShell,/data-dock-route="families"[^>]*><span>Families<\/span>/);assert.match(navShell,/data-dock-search/);assert.match(navShell,/<a href="#media">Photos<\/a>/);assert.match(navShell,/v158-mobile-more/);assert.match(navCss,/body\[data-v158-context="research"\]/);});
...
test('current experience runtime keeps dock navigation and freshness protection but drops legacy Home enhancers',()=>{assert.match(runtime,/#family-mobile-dock a\[href\^="#"\]/);assert.match(runtime,/event\.preventDefault\(\);navigate/);assert.match(runtime,/pageshow/);assert.match(runtime,/event\.persisted/);assert.match(runtime,/location\.reload\(\)/);assert.match(runtime,/build-info\.json\?ui-check=/);assert.match(runtime,/UI_RELEASE='17\.5\.0'/);assert.match(runtime,/STALE_RELOAD_KEY='family\.archive\.uiReload\.v17\.5'/);assert.doesNotMatch(runtime,/enhanceRecentPeople/);assert.doesNotMatch(runtime,/addDashboardPaths/);assert.doesNotMatch(runtime,/enhanceMediaMetric/);});
...
test('current production build emits one JavaScript bundle and one stylesheet with current shell stamping',()=>{assert.match(index,/app\.bundle\.js\?v=17\.5\.0/);assert.match(index,/styles\.css\?v=17\.5\.0/);assert.match(build,/const appVersion='17\.5\.0'/);assert.match(build,/shell\.replace\(/);assert.match(build,/outfile=dist\/app\.bundle\.js/);assert.match(build,/outfile=dist\/styles\.css/);assert.match(build,/bundler:`esbuild@\$\{ESBUILD_VERSION\}`/);assert.match(build,/browserAssets:\['app\.bundle\.js','styles\.css'\]/);});
