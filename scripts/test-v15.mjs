...
test('semantic stylesheet boundary preserves remaining compatibility cascade order',()=>{const expected=['v11.css','v11-nav.css','v11-2.css','v11-3.css','v11-4.css','v11-6.css','v12.css','v12-2.css','v12-3.css','v12-4.css','v12-5.css','v12-6.css','v12-6-1.css','v12-6-2.css','v12-7.css','v12-8.css','v12-9.css','v12-9-1.css','v13-0.css','dashboard-v13-2.css','media-page-v13-4.css','experience-v13-5.css','v14.css','v15.css','v15-1.css','v15-family-focus.css','platform-v13.css','v15-5.css','v15-6.css','v15-8.css'];const actual=[...styleRoot.matchAll(/@import ['"]\.\.\/\.\.\/([^'"]+\.css)['"]/g)].map(match=>match[1]);assert.deepEqual(actual,expected);assert.match(build,/src\/styles\/index\.css/);assert.doesNotMatch(build,/const cssSources=/);});
...
test('v17 experience runtime keeps dock navigation and freshness protection but drops legacy Home enhancers',()=>{assert.match(runtime,/#family-mobile-dock a\[href\^="#"\]/);assert.match(runtime,/event\.preventDefault\(\);navigate/);assert.match(runtime,/pageshow/);assert.match(runtime,/event\.persisted/);assert.match(runtime,/location\.reload\(\)/);assert.match(runtime,/build-info\.json\?ui-check=/);assert.match(runtime,/UI_RELEASE='17\.0\.0'/);assert.doesNotMatch(runtime,/enhanceRecentPeople/);assert.doesNotMatch(runtime,/addDashboardPaths/);assert.doesNotMatch(runtime,/enhanceMediaMetric/);});
...
test('v17 production build emits one JavaScript bundle and one unversioned stylesheet asset name',()=>{assert.match(index,/app\.bundle\.js\?v=17\.0\.0/);assert.match(index,/styles\.css\?v=17\.0\.0/);assert.match(build,/outfile=dist\/app\.bundle\.js/);assert.match(build,/outfile=dist\/styles\.css/);assert.match(build,/bundler:`esbuild@\$\{ESBUILD_VERSION\}`/);assert.match(build,/browserAssets:\['app\.bundle\.js','styles\.css'\]/);});
