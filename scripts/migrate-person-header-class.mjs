...
const legacy='v17-person-header';
const semantic='person-header';
...
const excludedFiles=new Set(['scripts/migrate-person-header-class.mjs']);
...
for(const full of walk(root)){
  ...
  const next=original.replace(exact,semantic)
    .replace(/\bperson-header\s+person-header\b/g,'person-header')
    .replace(/\.person-header\s*,\s*\.person-header(?![-_a-zA-Z0-9])/g,'.person-header');
