...
const legacy='v17-person-card';
const semantic='person-card';
...
const excludedFiles=new Set(['scripts/migrate-person-card-class.mjs']);
...
for(const full of walk(root)){
  ...
  const next=original.replace(exact,semantic)
    .replace(/\bperson-card\s+person-card\b/g,'person-card')
    .replace(/\.person-card\s*,\s*\.person-card(?![-_a-zA-Z0-9])/g,'.person-card');
