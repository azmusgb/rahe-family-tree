...
const stable=['scripts/test.mjs','scripts/test-branch-index.mjs','scripts/test-canonical-integrity.mjs','scripts/test-exports.mjs','scripts/test-record-ingestion.mjs','scripts/test-tree-advanced.mjs','scripts/test-research-automation.mjs'];
const capability=readdirSync('scripts').filter(n=>n.startsWith('contract-')&&n.endsWith('.mjs')).map(n=>`scripts/${n}`).sort();
run('contracts',['--test',...stable,...capability]);
const rootJs=readdirSync('.',{withFileTypes:true}).filter(e=>e.isFile()&&e.name.endsWith('.js')).map(e=>e.name);for(const file of [...rootJs,...walk('src','.js')])run('syntax',['--check',file]);
