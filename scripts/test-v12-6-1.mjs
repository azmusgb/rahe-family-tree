...
const release=String(model.meta.release||''),auditVersion=String(audit.version||'');const later=release==='12.6.1'||release==='12.6.2',laterAudit=auditVersion==='12.6.1'||auditVersion==='12.6.2';
test('v12.6.1 family polish remains active',()=>{assert(later);assert.match(model.familyExperience.profilePriority,/PERSON.*IMMEDIATE FAMILY.*LIFE STORY.*MEDIA/i);});
...
test('v12.6.1 semantic gates remain passing',()=>{for(const id of['AUD-054','AUD-055','AUD-056','AUD-057'])assert.equal(audit.checks.find(c=>c.id===id)?.pass,true,id);assert(laterAudit);assert.equal(audit.pass,true);});
