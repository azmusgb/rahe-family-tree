...
test('v14 design-system gains remain compatibility input under the v17 semantic release',()=>{assert.equal((index.match(/<link rel="stylesheet"/g)||[]).length,1);assert.match(index,/styles\.css\?v=17\.0\.0/);assert.match(index,/FAMILY VIEW · v17\.0\.0/);for(const token of['--space-4','--radius-md','--shadow-md','--mobile-nav-height'])assert.match(v14,new RegExp(token));assert.match(styleRoot,/\.\.\/\.\.\/v14\.css/);});

test('current browser runtime uses one cache-busted v17 production bundle',()=>{const scripts=[...index.matchAll(/<script type="module" src="([^"]+)"/g)].map(m=>m[1]);assert.deepEqual(scripts,['app.bundle.js?v=17.0.0']);assert.match(entry,/src\/runtime\/index\.js/);for(const domain of['base','media-core','deployment','family','media','tree','search','media-page','experience'])assert.match(runtimeIndex,new RegExp(`import './${domain}\\.js'`));assert.doesNotMatch(entry,/search-v13-2\.js/);assert.doesNotMatch(entry,/v15-runtime\.js/);const expected=['experience-core','../../v15-1-runtime','../../v15-family-focus','../../platform-v13-runtime','page-architecture','navigation-shell','native-family-v17-controller'];const actual=[...experience.matchAll(/import ['"]([^'"]+)\.js['"]/g)].map(match=>match[1].replace(/^\.\//,''));assert.deepEqual(actual,expected);});
