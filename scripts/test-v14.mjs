...
test('current browser runtime uses one cache-busted production entry with critical controllers eager',()=>{const version=releaseOf(index,/data-ui-release="(\d+\.\d+\.\d+)"/),scripts=[...index.matchAll(/<script type="module" src="([^"]+)"/g)].map(m=>m[1]);assert.deepEqual(scripts,[`app.bundle.js?v=${version}`]);assert.match(entry,/src\/runtime\/index\.js/);for(const domain of['base','media-core','deployment','family','media','tree','search','media-page','experience'])assert.match(runtimeIndex,new RegExp(`import './${domain}\\.js'`));assert.doesNotMatch(entry,/search-v13-2\.js/);assert.doesNotMatch(entry,/v15-runtime\.js/);const expected=['experience-core','../../v15-family-focus','../../platform-v13-runtime','page-architecture','navigation-shell','performance-contracts','native-family-v17-controller','tree-controller','mobile-ui-ownership','mobile-experience','mobile-route-state','mobile-ui-shell','mobile-ui-transient','mobile-ui-phase7'];const actual=[...experience.matchAll(/import ['"]([^'"]+)\.js['"]/g)].map(match=>match[1].replace(/^\.\//,''));assert.deepEqual(actual,expected);assert.doesNotMatch(experience,/v15-1-runtime\.js/);assert.match(routeCapabilities,/name:'unified-family-experience'[\s\S]*routes:\['dashboard','tree'\]/);assert.doesNotMatch(experience,/import\('\.\/unified-family-experience\.js'\)/);assert.match(experience,/tree-controller\.js/);assert.match(treeController,/v17-6-stability\.js/);assert.match(treeController,/tree-advanced\.js/);});
