...
test('v14 design-system gains remain compatibility input under the v17.1 semantic release',()=>{assert.equal((index.match(/<link rel="stylesheet"/g)||[]).length,1);assert.match(index,/styles\.css\?v=17\.1\.0/);assert.match(index,/FAMILY VIEW · v17\.1\.0/);for(const token of['--space-4','--radius-md','--shadow-md','--mobile-nav-height'])assert.match(v14,new RegExp(token));assert.match(styleRoot,/\.\.\/\.\.\/v14\.css/);});

test('current browser runtime uses one cache-busted v17.1 production bundle',()=>{const scripts=[...index.matchAll(/<script type="module" src="([^"]+)"/g)].map(m=>m[1]);assert.deepEqual(scripts,['app.bundle.js?v=17.1.0']);assert.match(entry,/src\/runtime\/index\.js/);for(const domain of['base','media-core','deployment','family','media','tree','search','media-page','experience'])assert.match(runtimeIndex,new RegExp(`import './${domain}\\.js'`));assert.doesNotMatch(entry,/search-v13-2\.js/);assert.doesNotMatch(entry,/v15-runtime\.js/);const expected=['experience-core','../../v15-1-runtime','../../v15-family-focus','../../platform-v13-runtime','page-architecture','navigation-shell','native-family-v17-controller'];const actual=[...experience.matchAll(/import ['"]([^'"]+)\.js['"]/g)].map(match=>match[1].replace(/^\.\//,''));assert.deepEqual(actual,expected);});
...
test('production build bundles JavaScript and rationalizes remaining compatibility CSS into one semantic asset',()=>{assert.match(build,/ESBUILD_VERSION='0\.25\.10'/);assert.match(build,/app-entry\.js/);assert.match(build,/--bundle/);assert.match(build,/outfile=dist\/app\.bundle\.js/);assert.match(build,/src\/styles\/index\.css/);for(const sheet of['v14.css','v15.css','v15-1.css','v15-5.css','v15-6.css','v15-8.css'])assert.match(styleRoot,new RegExp(sheet.replace('.','\\.')));for(const retired of['v15-7.css','v15-9.css','v15-10.css'])assert.doesNotMatch(styleRoot,new RegExp(`@import ['"][^'"]*${retired.replace('.','\\.')}`));for(const semantic of['tokens.css','base.css','home.css','people.css','person.css','tree.css','mobile-family.css','responsive.css'])assert.match(styleRoot,new RegExp(`@import './${semantic.replace('.','\\.')}'`));assert.match(build,/outfile=dist\/styles\.css/);assert.match(build,/browserAssets:\['app\.bundle\.js','styles\.css'\]/);});
...
test('legacy mobile navigation remains retired and the persistent dock stays static in the shell',()=>{assert.doesNotMatch(portraitRuntime,/createElement\(['"]nav['"]\).*mobile-family-nav/);assert.match(portraitRuntime,/removeLegacyMobileNav/);assert.match(index,/id="family-mobile-dock"/);assert.match(index,/Home<\/span>.*Tree<\/span>.*Search<\/span>.*People<\/span>.*Media<\/span>/s);});
