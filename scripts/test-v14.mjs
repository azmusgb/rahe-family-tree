...
test('current browser runtime uses one cache-busted production bundle',()=>{const scripts=[...index.matchAll(/<script type="module" src="([^"]+)"/g)].map(m=>m[1]);assert.deepEqual(scripts,['app.bundle.js?v=15.6.0']);for(const moduleName of['v11.js','media-page-v13-5.js','v15-1-runtime.js','v15-family-focus.js','platform-v13-runtime.js'])assert.match(entry,new RegExp(moduleName.replaceAll('.','\\.')));assert.doesNotMatch(entry,/search-v13-2\.js/);assert.doesNotMatch(entry,/v15-runtime\.js/);assert.match(runtimeIndex,/import '\.\/search\.js'/);assert.match(runtimeIndex,/import '\.\/experience\.js'/);const expected=['experience-core','../../v15-1-runtime','../../v15-family-focus','../../platform-v13-runtime','page-architecture','home-flow','navigation-shell','people-person-experience'];const actual=[...experience.matchAll(/import ['"]([^'"]+)\.js['"]/g)].map(match=>match[1].replace(/^\.\//,''));assert.deepEqual(actual,expected);});
