...
test('current browser runtime uses one stable cache-busted entry with split-capable experience modules',()=>{const version=releaseOf(index,/data-ui-release="(\d+\.\d+\.\d+)"/),scripts=[...index.matchAll(/<script type="module" src="([^"]+)"/g)].map(m=>m[1]);assert.deepEqual(scripts,[`app.bundle.js?v=${version}`]);assert.match(entry,/src\/runtime\/index\.js/);for(const domain of['base','media-core','deployment','family','media','tree','search','media-page','experience'])assert.match(runtimeIndex,new RegExp(`import './${domain}\\.js'`));assert.doesNotMatch(entry,/search-v13-2\.js/);assert.doesNotMatch(entry,/v15-runtime\.js/);const expected=['experience-core','../../v15-family-focus','../../platform-v13-runtime','page-architecture','navigation-shell','native-family-v17-controller'];const actual=[...experience.matchAll(/import ['"]([^'"]+)\.js['"]/g)].map(match=>match[1].replace(/^\.\//,''));assert.deepEqual(actual,expected);assert.doesNotMatch(experience,/v15-1-runtime\.js/);assert.match(experience,/unified-family-experience\.js/);assert.match(experience,/tree-controller\.js/);assert.match(treeController,/v17-6-stability\.js/);assert.match(treeController,/tree-advanced\.js/);});
