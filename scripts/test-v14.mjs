...
test('current browser runtime uses one cache-busted production entry with canonical app composition',()=>{const version=releaseOf(index,/data-ui-release="(\d+\.\d+\.\d+)"/),scripts=[...index.matchAll(/<script type="module" src="([^"]+)"/g)].map(m=>m[1]);assert.deepEqual(scripts,[`app.bundle.js?v=${version}`]);assert.match(entry,/src\/runtime\/index\.js/);assert.match(runtimeIndex,/import '\.\.\/app\/runtime\.js'/);const expected=['../../v11.js','../runtime/base-controls.js','../../media.js','../../deployment.js','../features/family/index.js','../features/media/index.js','../features/tree/index.js','../features/search/index.js','../../media-page-v13-5.js','../runtime/experience.js'];const actual=[...appRuntime.matchAll(/import ['"]([^'"]+\.js)['"]/g)].map(match=>match[1]);assert.deepEqual(actual,expected);assert.doesNotMatch(entry,/search-v13-2\.js/);assert.doesNotMatch(entry,/v15-runtime\.js/);const eager=['experience-core','../../v15-family-focus','../../platform-v13-runtime','page-architecture','navigation-shell','performance-contracts','native-family-v17-controller','tree-controller','mobile-ui-ownership','mobile-experience','mobile-route-state','mobile-ui-shell','mobile-ui-transient','mobile-ui-phase7'];const experienceImports=[...experience.matchAll(/import ['"]([^'"]+)\.js['"]/g)].map(match=>match[1].replace(/^\.\//,''));assert.deepEqual(experienceImports,eager);assert.doesNotMatch(experience,/v15-1-runtime\.js/);assert.match(routeCapabilities,/name:'unified-family-experience'[\s\S]*routes:\['dashboard','tree'\]/);assert.doesNotMatch(experience,/import\('\.\/unified-family-experience\.js'\)/);assert.match(treeController,/v17-6-stability\.js/);assert.match(treeController,/tree-advanced\.js/);});
