...
test('v12.6.2 visual QA is active',()=>{assert.equal(model.meta.release,'12.6.2');assert.equal(model.familyExperience.version,'12.6.2');assert.match(model.familyExperience.qaRule,/PRESENTATION-ONLY/i);});
...
test('v12.6.2 semantic gates pass',()=>{for(const id of['AUD-058','AUD-059','AUD-060','AUD-061'])assert.equal(audit.checks.find(c=>c.id===id)?.pass,true,id);assert.equal(audit.version,'12.6.2');assert.equal(audit.pass,true);});
