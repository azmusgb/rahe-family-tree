...
export const routes={
  dashboard:['Dashboard','Research state, evidence posture, priority acquisitions, and completeness controls.'],
  tree:['Tree','Connected family research graph with source-controlled relationship states and explicit uncertain links.'],
  people:['People','Appendix F person and identity inventory with stable IDs and research dossiers.'],
  branches:['Branches','Branch-oriented navigation across people, claims, tasks, and canonical dossier sections.'],
  timeline:['Timeline','Source-dated rows rendered as a research chronology. Event type is a display classification only.'],
  ...
  research:['Research Queue','Priority record acquisition, branch sequences, stop rules, and negative-search controls.'],
...
export function personById(id){return model.people.find(p=>p.id===id);}
...
export const personCard=p=>`<article class="person-card state-${stateClass(p.state)}"><button data-person="${p.id}" class="person-open"><span class="eyebrow">${esc(p.branch)} · ${esc(p.id)}</span><h3>${esc(p.name)}</h3><p>${esc(p.dates)}</p><div>${stateBadges(p.state)}</div><small>${esc(p.role)}</small></button></article>`;
...
export function globalSearchResults(){const q=currentFilters().q;if(!q)return'';const qn=norm(q);const groups=[['person',model.people,p=>p.name,p=>`${p.branch} · ${p.state}`],['claim',model.claims,c=>c.claim,c=>`${c.id} · ${c.state}`],['source',model.sources,s=>s.name,s=>`${s.id} · ${s.weight}`],['task',model.researchTasks,t=>t.record,t=>`${t.priority} · ${t.branch}`]];let total=0;let html='';for(const[k,arr,title,meta]of groups){const hits=arr.filter(x=>norm(JSON.stringify(x)).includes(qn)).slice(0,8);total+=hits.length;html+=hits.map(x=>`<button class="search-hit" data-${k}="${esc(x.id)}"><span>${k.toUpperCase()}</span><b>${esc(title(x))}</b><small>${esc(meta(x))}</small></button>`).join('');}const sections=corpus.sections.filter(s=>norm(JSON.stringify(s)).includes(qn)).slice(0,10);total+=sections.length;html+=sections.map(s=>`<a class="search-hit" href="#archive/${s.id}"><span>SECTION</span><b>${esc(s.title)}</b><small>${s.legacy?'Legacy annex':'Canonical/control'} · ${s.id}</small></a>`).join('');return`<section class="search-results" aria-label="Global search results"><div class="section-title"><div><p class="eyebrow">GLOBAL SEARCH</p><h2>Matches for “${esc(q)}”</h2></div><span>${total} top matches</span></div><div class="search-grid">${html}</div></section>`;}
