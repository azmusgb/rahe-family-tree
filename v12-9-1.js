...
function breadcrumb(){
  ...
  const focus=focusId(),home=localStorage.getItem(HOME_KEY)||'',recent=recentIds().filter(id=>id!==focus);
...
function installMobileHint(){if(route()!=='tree')return;const scroll=document.querySelector('.graph-scroll');if(!scroll||document.querySelector('.v1291-mobile-hint'))return;const hint=document.createElement('div');hint.className='v1291-mobile-hint';hint.textContent='Tap a person to open their profile. Use the tree controls to change focus, depth, and family scope.';scroll.insertAdjacentElement('beforebegin',hint);}
function syncRelease(){document.body.dataset.treePolishRelease='12.9.1';const edition=document.querySelector('.edition');if(edition){const current=edition.innerHTML,next=current.replace(/Family experience v12\.9(?:\.1)?|Family experience v12\.8/g,'Family experience v12.9.1');if(next!==current)edition.innerHTML=next;}const version=document.querySelector('.version');if(version){const current=version.textContent||'',next=current.replace(/v12\.\d+(?:\.\d+)?/,'v14.0');if(next!==current)version.textContent=next;}}
